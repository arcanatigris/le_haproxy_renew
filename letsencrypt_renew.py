...
for filename in os.listdir(certs_location):
    ...
    if remaining > renew_threshold:
        logging.warning('The certificate for %s is up to date, no need for renewal (%d days left).',filename[:-4],remaining)
    else:
        logging.warning('The certificate for %s is about to expire soon. Starting Let\'s Encrypt (HAProxy:%s) renewal script...',filename[:-4],http_01_port)
        renew = renew + 1
        ...
        os.popen('cat /etc/letsencrypt/live/%s/fullchain.pem /etc/letsencrypt/live/%s/privkey.pem > %s%s' % (filename[:-4],filename[:-4],certs_location,filename))
